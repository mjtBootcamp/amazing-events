      ...
      if (textInputValue == "") {
        ...
        if (checkedCategories.length != 0) {
          filtedEvents.forEach(event => {
            ...
            //filtrar por texto
            let filtedEventsTextAndCategory = filtedEventsCategory.filter(event => event.name.toLowerCase().includes(text.toLowerCase()))
            setFiltedEvents(filtedEventsTextAndCategory)
